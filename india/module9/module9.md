...
*   The coordinate reference system used is the PRS92 / Philippines zone 3, EPSG 24383. 
...
Don’t forget to also add the boundary of the working area, district of Bangalore, which is a vector dataset. 
...
For layer HRSL_Bangalore_Population, the information window would look like in figure 9.7. 
...
If you selected the output to be **[Save to temporary file]** then there will be a raster layer named `Reprojected` in the Layers Panel. This is a memory layer and you can rename this layer to Reprojected_HRSL_Bangalore_Population and save it to make it persistent.
...
```
        ...
        PROJCRS["Kalianpur 1975 / India zone IVa",
            BASEGEOGCRS["Kalianpur 1975",
                ...
                ID["EPSG",4146]],
            CONVERSION["India zone IVa (1975 metres)",
                METHOD["Lambert Conic Conformal (1SP)",
                    ID["EPSG",9801]],
                PARAMETER["Latitude of natural origin",12,
                    ANGLEUNIT["degree",0.0174532925199433],
                    ID["EPSG",8801]],
            ...
            USAGE[
                ...
                AREA["India - mainland onshore south of 15°N."],
                BBOX[8.02,73.94,15.01,80.4]],
            ID["EPSG",24383]]
        Data axis to CRS axis mapping: 1,2
        Origin = (2452500.645106573589146,1051067.953913605306298)
        ...
        INTERLEAVE=PIXEL
        Corner Coordinates:
        Upper Left  ( 2452500.645, 1051067.954) ( 77d18'53.76"E, 13d13'26.73"N)
        Lower Left  ( 2452500.645,  986467.954) ( 77d19'14.64"E, 12d38'22.25"N)
        Upper Right ( 2508100.645, 1051067.954) ( 77d49'42.50"E, 13d13'42.69"N)
        Lower Right ( 2508100.645,  986467.954) ( 77d49'59.39"E, 12d38'38.18"N)
        Center      ( 2480300.645, 1018767.954) ( 77d34'27.57"E, 12d56' 2.94"N)
        Band 1 Block=556x4 Type=Byte, ColorInterp=Red
        Description = discrete_classification
            Computed Min/Max=20.000,126.000
        Minimum=20.000, Maximum=126.000, Mean=56.252, StdDev=33.967
        NoData Value=225
        Metadata:
            STATISTICS_MAXIMUM=126
            STATISTICS_MEAN=56.25223319962
            STATISTICS_MINIMUM=20
            STATISTICS_STDDEV=33.967066661819
            STATISTICS_VALID_PERCENT=98.21
        Band 2 Block=556x4 Type=Byte, ColorInterp=Green
        Description = forest_type
            Computed Min/Max=0.000,4.000
        Minimum=0.000, Maximum=4.000, Mean=0.225, StdDev=0.887
        NoData Value=225
        Metadata:
            STATISTICS_MAXIMUM=4
            STATISTICS_MEAN=0.22534692199748
            STATISTICS_MINIMUM=0
            STATISTICS_STDDEV=0.8871954853345
            STATISTICS_VALID_PERCENT=98.21
        Band 3 Block=556x4 Type=Byte, ColorInterp=Blue
        Description = urban-coverfraction
            Computed Min/Max=0.000,100.000
        Minimum=0.000, Maximum=100.000, Mean=22.164, StdDev=37.515
        NoData Value=225
        Metadata:
            STATISTICS_MAXIMUM=100
            STATISTICS_MEAN=22.164430395895
            STATISTICS_MINIMUM=0
            STATISTICS_STDDEV=37.515388299508
            STATISTICS_VALID_PERCENT=98.21
```
...
![Zooming in on the DSM_mosaic_24383 computed histogram](media/fig918.png "Zooming in on the DSM_mosaic_24383 computed histogram")
...
![Batch process cliping all required raster layers by Bangalore Province geometry](media/fig923.png "Batch process cliping all required raster layers by Bangalore Province geometry")
...
In order to calculate the number of people based on the raster data HRSL that live up to 900 meters in Bangalore district, we must see which pixels fall in each of those categories. To do that, we will use **Raster Calculator**. This is a functionality allowing the user to perform calculations on the basis of existing raster pixel values. The results are written to a new raster layer in a GDAL[^5]-supported format.
...
Next, we will ‘slice’ our DSM_mosaic_clipped raster layer to extract only the pixels with values up to 900 meters. We know that the value cells of the DSM-mosaic_clipped represent a continuous numerical data (not discrete values, such as LandCover). Therefore, the operation we need to employ in this case is a comparison one - cell values <= 900 meters. To obtain this, we will write the following the Raster Calculator: 

```
"Clipped_Reprojected_Merged_SRTM@1" <= 900
``` 
...
![Result of identify all pixel values that are below 900 meters using the Raster Calculator](media/fig928.png "Result of identify all pixel values that are below 900 meters using the Raster Calculator")

Figure 9.28 - Result of identify all pixel values that are below 900 meters using the Raster Calculator

The result will be named `Output`. You can rename this to `< 200`. As we can see in the Layers Panel, the raster layer we have obtained has only 2 values 0 and 1. That is because we have used a rational operation, a comparison, therefore every pixel that is below 900 meters received value = 1 and all above, value = 0. We can test this by using the `Value Tool`. Figure 9.29 presents only pixels of value 1, in other words the pixels we are interested in for our exercise. 


![Spatial distribution of all pixels of value 1, meaning with altitude lower than 900 meters](media/fig929.png "Spatial distribution of all pixels of value 1, meaning with altitude lower than 900 meters")

Figure 9.29 - Spatial distribution of all pixels of value 1, meaning with altitude lower than 900 meters 

Going further, we can show the spatial distribution of population at the 30 m spatial resolution only in this specific geographical region, we’ve selected - Bangalore district, below 900m. To do that, we again employ Raster Calculator. 
...
```
"< 900@1"*"Reprojected_HRSL_Bangalore_Population@1"
```


![Using raster calculator to identify population distribution classes based on altitude of up to 900m](media/fig930.png "Using raster calculator to identify population distribution classes based on altitude of up to 900m")

Figure 9.30 - Using raster calculator to identify population distribution classes based on altitude of up to 900m
...
You can see that even if Reprojected_HRSL_Bangalore_Population has values in this specific mouse location, the raster obtained with Raster Calculator HRSL_DSM has value 0. 

Next, we present the spatial distribution of the population that lives below 900m in Bangalore district. To choose an appropriate classification, we calculate the histogram. We can notice that most values are between 0.1 and 200 people per 30m. The classification we’ve chosen is visible in figure 9.32. 


![Distribution of population that lives below 900m in Bangalore district, represented at a 30m resolution](media/fig932.png "Distribution of population that lives below 900m in Bangalore district, represented at a 30m resolution")

Figure 9.32 - Distribution of population that lives below 900m in Bangalore district, represented at a 30m resolution. 

If we are interested in the total number of people living below 900m in Bangalore district and not the geographical distribution per 30 m spatial resolution, then we need to sum up all pixel values of the raster layer HRSL_DSM. One way to obtain this number is to transform the DSM_clipped200 from raster to vector and them [....]
...
And with this final step, we answered our exercise, how many people (and where) are living below 900m in Bangalore district. 
...
In the previous phase, we have seen how we can process 2 raster datasets in order to derive new information. We have used the digital surface model and the High Resolution Settlement Layer to find out how many people live below 900m in Bangalore district. Before doing any analysis, we made sure that the datasets were in the same projection and, furthermore if the rasters have the same spatial resolution so that the results we obtained are viable. When referring to the coordinate reference system, the reasoning is clear, but why the same spatial resolution? 
...
Figure 9.40  - Sentinel-2 cloudless layer for 2019 by EOX - 4326 for Bangalore district
...
Figure 9.51 - LC2019_Mode polygon
...
Considering the extent of your study area, this operation can be significantly lengthy in time. Figure 9.54 shows how many points we have obtained for Bangalore district.
...
Figure 9.56 - Raster tiles for the Bangalore district HRSL 
