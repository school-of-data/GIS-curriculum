...
*   ETH_Addis_Ababa and ETH_Bole vector layers (inside [module3.gpkg](module3.gpkg)).
...
or this exercise, we will load road network data and locations of fast food chains from OSM that can be found in the province of Addis Ababa using the QuickOSM plugin.
...
1. Load the ETH_Addis_Ababa and ETH_Bole vector layers found inside the module3 geopackage.

![Addis Ababa layers loaded in QGIS](media/quickosm-1.png "Addis Ababa layers loaded in QGIS")

Figure 3.2. Addis Ababa layers loaded in QGIS
...
4. Load all highways inside the extent of the ETH_Bole layer.

    * Key: highway
    * Value: &lt;blank> (blank means ALL)
    * Layer Extent: ETH_Bole
    * Advanced:
        * Check Node, Way, Relation, Lines, Multilinestrings

![Load all highways in the ETH_Bole layer extent](media/quickosm-3.png "Load all highways in the ETH_Bole layer extent")

Figure 3.4. Load all highways in the ETH_Bole layer extent
...
8. Next, let’s load all fast food restaurant locations in the ETH_Addis_Ababa layer extent. Open the QuickOSM plugin and put the following parameters in the Quick query tab:

    * Key: amenity
    * Value: fast_food
    * Layer Extent: ETH_Addis_Ababa
    * Advanced:
        * Check Node, Way, Relation, Points

![Load amenities (points) tagged as fast_food in the ETH_Addis_Ababa layer extent](media/quickosm-6.png "Load amenities (points) tagged as fast_food in the ETH_Addis_Ababa layer extent")

Figure 3.7: Load amenities (points) tagged as fast_food in the ETH_Addis_Ababa layer extent
...
1. Load the **HRSL_Addis_AbabaPopulation** raster file in QGIS.

![The HRSL for Addis Ababa, Sri Lanka](media/hrsl-1.png "The HRSL for Addis Ababa, Sri Lanka")

Figure 3.10: The HRSL for Addis Ababa, Sri Lanka
...
Using an Overpass query allows you to create more complex feature selection in QuickOSM. For example, let’s load all fast foods again inside the extent of ETH_Addis_Ababa but this time let’s just get fast foods with Burger in their name. If you look at the attribute table of fast food layer, you will notice that it has a **name** field. The fields in the attribute table of data loaded from OSM correspond to the tag keys so if we want to just select all Burger King fast food branches, we need to add a filter that selects a feature if it has a **key:value** of **name: has burger**. This filter can easily be added in an Overpass API.

1. Open the QuickOSM plugin and input the parameters we used for querying all fast foods.

![QuickOSM parameters for loading fast foods in Addis Ababa](media/quickosm-6.png "QuickOSM parameters for loading fast foods in Addis Ababa")

Quick query to load all amenities tagged as fast food

2. Click Show query.

![Overpass version of the query](media/quickosm-6.png "Overpass version of the query")

Overpass to load all amenities tagged as fast food

3. Edit the query and add the line **&lt;has-kv k="name" regv="Burger"/>** after each &lt;has-kv k="amenity" v="fast_food"/> line.

![Overpass query to load only fast foods with Burger in their name](media/overpass-2.png "Overpass query to load only fast foods with Burger in their name")

Overpass query to load all amenities tagged as fast food whose name has Burger

4. Click Run query. This should load just the amenities tagged as fast_food with the name Burger King. The layer name will be OsmQuery.

![Amenities tagged as fast food whose name has Burger loaded from OSM](media/overpass-3.png "Amenities tagged as fast food whose name has Burger loaded from OSM")

Amenities tagged as fast food whose name has Burger loaded from OSM
