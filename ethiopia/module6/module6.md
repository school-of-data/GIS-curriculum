...
*   Internet connection
*   QGIS 3.16 and above
*   Addis Ababa administrative boundary layer (inside [module6.gpkg](data/module6.gpkg))
*   Addis Ababa clinics (inside [module6.gpkg](data/module6.gpkg))
*   ETH zones (inside [module6.gpkg](data/module6.gpkg))
...
In some cases, for instance for infrastructure planning purposes, you may want to know the area of the polygons in a layer. If you have one polygon, this would not be a problem. But what if you have many polygons/areas in the layer? To calculate each area one by one is almost impossible. The Ethiopia administrative boundary vector layer has many polygons which means it would be a good dataset for this tutorial. We are going to calculate the area for each polygon using an automated way. In total, the area of 79 zones will be calculated. 
...
1. Load the ETH zones vector layer (found inside [module6.gpkg](data/module6.gpkg)) in QGIS. You can know how many features are in the current feature by **Right-clicking on the layer on the Layers Panel ‣ Show Feature Count**. As you can see below, the vector layer has many features. In total, there are 79 features corresponding to 79 zones.
...
3. For area calculations, the Coordinate Reference System should be a projected one. This allows you to calculate distances correctly. Remember, our interest is to automatically calculate the area for each of the 79 zones. Check the Coordinate Reference System of the vector layer. If it’s a geographic coordinate reference system, then reproject the layer to a projected coordinate system. Check for different projections on the [EPSG](https://epsg.io/?q=Ethiopia%20kind%3APROJCRS) website. This is the Ethiopia, we’ll use [Adindan / UTM Zone 37N](https://epsg.io/20137), EPSG:20137. From previous modules where map projections are extensively discussed, you may already know that map projections are applied relative to a given location on earth. 
...
At this point, you might notice that the attribute table stores both spatial and non-spatial data. In this tutorial you’ll discover ways of working with attribute table data. For example, Using expressions, select Clinics in Addis Ababa with an emergency facility, all using data from the attribute table. 

1. Add the following datasets of the Ethiopia to the QGIS map canvas; 
...
You’ll notice that the expression has a number of predicates like the comparison sign (=, !=), the logical predicate (AND) and a string which is enclosed in single quotes (''). There are also two attribute names(amenity, phone) and their values (clinic, ''). 

![alt_text](media/select.png "image_tooltip")

Figure 6.8: Select by Expression builder dialogue

3. One Clinic is selected. You can see the selection highlighted in yellow. The selected clinic is also highlighted in the attribute table. 
