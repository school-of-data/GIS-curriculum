...
Minimum value: 0.92
Maximum value: 16680.73
Range: 16679.81
Sum: 7605471.9099999545
Mean value: 170.15239854132074
Median value: 98.43
Standard deviation: 303.25292119510505
Coefficient of Variation: 1.7822429997744724
Minority (rarest occurring value): 0.92
Majority (most frequently occurring value): 29.88
First quartile: 51.37
Third quartile: 186.22
Interquartile Range (IQR): 134.85
```

From these basic statistics, we find out that there are 44698 road segments in the loaded layer, where the shortest has 0.92 m and the longest 16680.73 m - almost 17 km. We find out that the sum of roads in Addis Ababa is almost 7.6k km (7605.471 km). Given that the mean is greater than the median, it tells us that the 2nd half of the dataset contains longer road segments and that it outweighs the road segments in the 1st half. However, the median shows that most road segments have length around 116 m. 
...
Count: 12088
Unique values: 42
NULL (missing) values: 8286
Minimum value: Church
Maximum value: warehouse
Minimum length: 0
Maximum length: 18
Mean length: 2.9171906022501655
```


The results don’t look the same, we don’t have mean, nor median or standard deviation. That is because the attribute field we ran the algorithm on is different, we don’t have numbers but words - types of buildings. We find out that out of 12088 buildings in Addis Ababa, for 8286 we don’t know the type of the building. We also find out that there are 42 unique categories. 
...
We will set a number of rules for the layers we have loaded in our QGIS project, considering the real world objects they depict- roads, buildings, waterways in the city of Addis Ababa. 
...
Figure 8.17 - Topology checker rules on the buildings vector layer
...
Figure 8.18 - Results of topology check on the buildings vector layer

Fortunately, it looks like there are no duplicates to remove in the buildings layer.
...
Let's run one more algorithm to get a sense of what the attributes for our Addis Ababa layers are. After we’ve identified how many features each layer has, let’s see how many and which are the unique attributes in the following cases: 
...
<table>
  ...
  <tr>
   ...
   <td>cathedral;roof;warehouse;NULL;civic;administrative;hut;apartments;hotel_apartment;train_station;detached;parking;retail;building;school;hotel;Church;industrial;hangar;public;mall;mosque;no;service;NB BUILDING;church;barn;cultural_institute;stadium;office;store;house;kindergarten;residential;kiosk;Office_building;farm_auxiliary;commercial;Shop;construction;hospital;university
   </td>
  </tr>
  <tr>
   <td>pois_cleaned
   ...
   <td>water_tower;college;embassy;toy_shop;cafe;atm;artwork;jeweller;fountain;community_centre;tourist_info;nightclub;restaurant;motel;swimming_pool;hostel;florist;biergarten;public_building;park;kindergarten;pitch;car_sharing;bookshop;sports_centre;pharmacy;convenience;camera_surveillance;guesthouse;furniture_shop;doctors;attraction;bank;travel_agent;water_well;car_rental;dentist;telephone;optician;waste_basket;nursing_home;comms_tower;hotel;garden_centre;library;department_store;supermarket;mall;beverages;doityourself;gift_shop;picnic_site;chalet;castle;mobile_phone_shop;post_office;ruins;clothes;theatre;bar;chemist;toilet;shoe_shop;greengrocer;stationery;post_box;hairdresser;school;beauty_shop;viewpoint;fast_food;prison;veterinary;fire_station;camp_site;sports_shop;shelter;memorial;laundry;pub;car_dealership;food_court;bakery;courthouse;computer_shop;hospital;bench;university;monument;water_works;police;hunting_stand;butcher;town_hall;playground;bicycle_rental;video_shop;caravan_site;tower;museum;outdoor_shop;cinema;kiosk;drinking_water;car_wash;bicycle_shop;arts_centre
   </td>
  </tr>
  <tr>
   <td>waterways
  ...
  <tr>
   ...
   <td>christian_catholic;christian_lutheran;muslim;christian;muslim_sunni;christian_orthodox;christian_protestant
   </td>
  </tr>
  <tr>
   <td> roads 
   ...
   <td>pedestrian;primary;track_grade3;steps;track_grade5;trunk;secondary;tertiary;trunk_link;tertiary_link;footway;track;secondary_link;track_grade4;unclassified;service;motorway;residential;primary_link;track_grade2;motorway_link;path
   </td>
  </tr>
  <tr>
   <td>landuse
   ...
   <td>	quarry;meadow;park;grass;recreation_ground;military;forest;commercial;heath;farmyard;retail;cemetery;farmland;nature_reserve;industrial;residential
   </td>
  </tr>
</table>
...
**Buffer.** Imagine that you need to analyse a new piece of legislation that asks that on an area of 500 meters around places of worship there can be no other construction built. You would want to see where exactly those delinations are and maybe even how many square meters that is for your city. First step is to define a buffer around the places of worship: **Vector ‣ geoprocessing tools ‣ Buffer**. When the buffer window opens, set the parameters like in figure 8.23: 
...
**Clip.** Imagine you want to know where all the industrial delineated areas are in your city and also how many buildings are within that perimeter. Visual inspecting your vector data, you notice that you have a number of industrial areas that contain several buildings. You want to separate those buildings and to use them further. First step is to select all features in the landuse layer that have as attribute industrial (see module 6 for how to do that). Afterwards, you go to **Vector ‣ Geoprocessing tools ‣ Clip** and choose as the layer to be clipped buildings_cleaned. Your results should look like in figure 8.27b. 
...
**Thiessen (Voronoi) polygons.** Imagine you have to make a series of administrative decisions in your city based on how many schools there are and what specific areas they serve. Geospatial analysis can be of assistance. You can start by calculating the Thiessen polygons. Based on an area containing at least two points, a Thiessen Polygon is a 2-dimensional shape which boundaries contain all space which is closer to a point within the area than any other point without the area. A good use example is in meteorology, where weather stations are discrete points, yet the information collected is considered to be measured out on the surface based on the thiessen polygons. 

To respond to the above question, we will run the algorithm only for points that have the attribute school at type. Thus, make the selection as instructed in module 6. You should have 88 features selected on layer pois_cleaned. Go to **Vector ‣ Geometry Tools ‣ Voronoi Polygons..** After setting the parameters - select the point layer for which we want the Voronoi polygons calculated and a 30% extension so that the entire Addis Ababa city is contained, you should see a result like in figure 8.28d. 
...
Let us assume that you have to present a report that will allow comparisons done for units of 5X5 km over the administrative unit, including: 
...
We’ve seen that there are tools that can assist us in calculating the total surface occupied by a certain type of feature, however the first step is to create our 5X5 units - cell grids. To do that, go to: **Vector ‣ Research Tools ‣ Create grid..** Set the parameters to: 
- Grid type - Rectangle (polygon)
- Grid extent - ETH_Addis_Ababa
- Horizontal spacing - 5 km
- Vertical spacing - 5 km 


![Creating a 5X5km vector grid for the Addis Ababa city](media/fig829_a.png "Creating a 5X5km vector grid for the Addis Ababa city")

Figure 8.29a - Create 10kmx10km vector grid for Addis Ababa city

You should get a result like in figure 8.29b. 


![5X5km vector grid for the Addis Ababa city](media/fig829_b.png "5X5km vector grid for the Addis Ababa city")

Figure 8.29b - 5X5km vector grid for the Addis Ababa city
...
![Filtering green areas and built-up space in Addis Ababa](media/fig830_a.png "Filtering green areas and built-up space in Addis Ababa ")

Figure 8.30a - Filtering green areas and built-up space in Addis Ababa 


![Spatial distribution of the green areas and built-up space in Addis Ababa](media/fig830_b.png "Spatial distribution of the green areas and built-up space in Addis Ababa")

Figure 8.30b - Spatial distribution of the green areas and built-up space in Addis Ababa 

The second step to answer the requirement, is to identify how much green space and how much built-up space there is in each 5X5 km. To obtain that we will **intersect** the 2 overlaid polygon vector layers. The algorithm extracts the overlapping portions of features in the Input - the landuse layer and Overlay layer - the grid layer. Go to **Vector - Geoprocessing Tools - Intersection** or look for **Intersection in the Processing Toolbox or Locator Bar** Set the algorithm parameters as in figure 8.31.
...
Now for each 5X5 km unit, we have the landuse features that we can work with. The attribute table also stores this information, as each grid cell - unit - has a unique id, see figure 8.33. 
...
![GroupStat parameters setup to sum up the green areas per each 5X5km grid cell](media/fig834_e.png "GroupStat parameters setup to sum up the green areas per each 5X5km grid cell")

Figure 8.34e - GroupStat parameters setup to sum up the green areas per each 5X5km grid cell.
...
Moving forward, we need to join the calculated spaces - green and built-up - to each 5X5 km cell grid. For that, select the grid10km vector layer in the TOC, open the properties window and go to **Joins**. This functionality allows you to join by a common attribute field, others. In our case, using the common grid_id value we will join, the sum of the built-up areas and green spaces fro the 2 csv files obtained in the previous stage.
...
![Setting the parameters to join by common field grid_id/id the sums of green and built-up spaces for each grid cell - 5X5km unit](media/fig835_c.png "Setting the parameters to join by common field grid_id/id the sums of green and built-up spaces for each grid cell - 5X5km unit")

Figure 8.35c - Setting the parameters to join by common field grid_id/id the sums of green and built-up spaces for each grid cell - 5X5km unit.
...
![Spatial distribution of 5X5km units with most roads](media/fig838_c.png "Spatial distribution of 5X5km units with most roads")

Figure 8.38c - Spatial distribution of 5X5km units with most roads
...
![Spatial distribution of public POIs density per unit 5X5km](media/fig840_d.png "Spatial distribution of public POIs density per unit 5X5km")


Figure 8.40d - Spatial distribution of public POIs density per unit 5X5km
...
Diving into the math behind each interpolation algorithm is beyond the scope of this module. However, for demonstration purposes, we will simulate precipitation data interpolation to obtain a seamless dataset on the quantity of precipitations fallen in our area of interest, Addis Ababa city. 
...
Now that we have our imaginary meteo stations that measure precipitations in Addis Ababa city, we will continue by adding fictitious measurements on the course of 7 days. 
...
![Fictitious precipitation data for the 93 fictitious meteo stations in Addis Ababa city](media/fig845.png "Fictitious precipitation data for the 93 fictitious meteo stations in Addis Ababa city")

Figure 8.45 - Fictitious precipitation data for the 93 fictitious meteo stations in Addis Ababa city.

Next, we will interpolate these values for each of the 7 days  to obtain a seamless layer that covers the entire territory of the city. Given that the operation is repetitive, we will use batch processing. The selected interpolation method selected - strictly for demonstration purposes! - is IDW - inverse distance weighted. 

Set the following parameters: 
* distance coefficient: 2
* extent: ETH_Addis_Ababa
...
The meteo stations are visible on the map canvas and in the TOC you can see all the 7 newly created raster datasets that represent precipitation values for each day in the Addis Ababa city. 
...
However, as we have interpolated values for 7 days, let us prepare a short animation on how precipitation values have evolved for the Addis Ababa city. 
