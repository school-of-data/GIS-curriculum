...
Don’t forget to also add the boundary of the working area, province of Gauteng, which is a vector dataset. 
...
For layer HRSL_Gauteng_Population, the information window would look like in figure 9.7. 
...
Thus, the first thing to do is to reproject all datasets we will work with in the same coordinate system - EPSG 102022.
...
If you selected the output to be **[Save to temporary file]** then there will be a raster layer named `Reprojected` in the Layers Panel. This is a memory layer and you can rename this layer to Reprojected_HRSL_Gauteng_Population and save it to make it persistent.
...
Now, we can proceed to reprojecting the mosaic - one file, instead of 6 files. Go to **Raster ‣ projection ‣ Wrap (reproject)** and set the known parameters: 
   ...
   * Target CRS: EPSG:102022 
   * Resampling method: Nearest neighbour
   * Output file resolution - 30 m. 

At his point, we should have all layers in the same CRS - EPSG 102022. 
...
Files: J:/sa_landcover/Reprojected_LandCover2018.tif
Size is 14301, 11913
...
Origin = (161674.388662100187503,-2840977.503469627350569)
Pixel Size = (27.012833538530913,-27.012833538530913)
Corner Coordinates:
Upper Left  (  161674.389,-2840977.503) ( 26d34'40.99"E, 24d30'27.23"S)
Lower Left  (  161674.389,-3162781.389) ( 26d34'48.59"E, 27d28'41.28"S)
Upper Right (  547984.921,-2840977.503) ( 30d20'55.32"E, 24d30' 8.52"S)
Lower Right (  547984.921,-3162781.389) ( 30d21'21.09"E, 27d28'22.10"S)
Center      (  354829.655,-3001879.446) ( 28d27'56.50"E, 25d58'56.10"S)
Band 1 Block=14301x1 Type=Byte, ColorInterp=Red
  Description = discrete_classification
    Computed Min/Max=0.000,126.000
  Minimum=0.000, Maximum=126.000, Mean=42.074, StdDev=31.669
Band 2 Block=14301x1 Type=Byte, ColorInterp=Green
  Description = forest_type
    Computed Min/Max=0.000,4.000
  Minimum=0.000, Maximum=4.000, Mean=0.020, StdDev=0.282
Band 3 Block=14301x1 Type=Byte, ColorInterp=Blue
  Description = urban-coverfraction
    Computed Min/Max=0.000,100.000
  Minimum=0.000, Maximum=100.000, Mean=3.954, StdDev=17.426
```
...
![Zooming in on the DSM_mosaic_102022 computed histogram](media/fig918.png "Zooming in on the DSM_mosaic_102022 computed histogram")

Figure 9.18 - Zooming in on the DSM_mosaic_102022 computed histogram
...
To work only with raster layers that are relevant to our Gauteng province, we will use the vector extent layer (Gauteng_admin_boundary) to cut/clip all relevant raster layers. Go to **Raster ‣ Extraction ‣ Clip Raster by Mask Layer** (see figure 9.22). Similarly, you can search for Clip in the Processing Toolbar or the Locator bar.
...
![Batch process cliping all required raster layers by Gauteng Province geometry](media/fig923.png "Batch process cliping all required raster layers by Gauteng Province geometry")

Figure 9.23 - Batch process cliping all required raster layers by Gauteng Province geometry

The parameters setup are the following:
* mask layer: Gauteng_admin_boundary
* both source and target CRS is EPSG 102022
...
In order to calculate the number of people based on the raster data HRSL that live up to 200 meters in Gauteng province, we must see which pixels fall in each of those categories. To do that, we will use **Raster Calculator**. This is a functionality allowing the user to perform calculations on the basis of existing raster pixel values. The results are written to a new raster layer in a GDAL[^5]-supported format.
...
Going further, we can show the spatial distribution of population at the 30 m spatial resolution only in this specific geographical region, we’ve selected - Gauteng province, below 200m. To do that, we again employ Raster Calculator. 
...
```
"< 200@1"*"Reprojected_HRSL_Gauteng_Population@1"
```
...
You can see that even if Reprojected_HRSL_Gauteng_Population has values in this specific mouse location, the raster obtained with Raster Calculator HRSL_DSM has value 0. 

Next, we present the spatial distribution of the population that lives below 200m in Gauteng province. To choose an appropriate classification, we calculate the histogram. We can notice that most values are between 0.1 and 200 people per 30m. The classification we’ve chosen is visible in figure 9.32. 


![Distribution of population that lives below 200m in Gauteng province, represented at a 30m resolution](media/fig932.png "Distribution of population that lives below 200m in Gauteng province, represented at a 30m resolution")

Figure 9.32 - Distribution of population that lives below 200m in Gauteng province, represented at a 30m resolution. 

If we are interested in the total number of people living below 200m in Gauteng province and not the geographical distribution per 30 m spatial resolution, then we need to sum up all pixel values of the raster layer HRSL_DSM. One way to obtain this number is to transform the DSM_clipped200 from raster to vector and them [....]
...
And with this final step, we answered our exercise, how many people (and where) are living below 200m in Gauteng province. 
...
Figure 9.40  - Sentinel-2 cloudless layer for 2019 by EOX - 4326 for Gauteng province
...
* Source CRS and Target CRS: EPSG: 102022, 
* Resampling method: Nearest Neighbour, 
...
Considering the extent of your study area, this operation can be significantly lengthy in time. Figure 9.54 shows how many points we have obtained for Gauteng province.
...
Figure 9.56 - Raster tiles for the Gauteng province HRSL 
