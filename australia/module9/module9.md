...
*   [module9.gpkg](data/module9.gpkg)
    *   Tasmania_level1
*   [High Resolution Settlement Layer](data/HRSL_AUS_Tasmania_Population.tif)
*   [SRTM Digital Elevation Model](data/SRTM_DEM)
*   [Global Land Cover Map 2015-2019](data/Global_Land_Cover_Map)
*   The coordinate reference system used is the Geocentric Datum of Australia 1994/ Map Grid of Australia zone 55 (GDA94 / MGA zone 55) EPSG 28355. 
...
Don’t forget to also add the boundary of the working area - Tasmania - which is a vector dataset. 
...
For layer HRSL_AUS_Tasmania_Population, the information window would look like in figure 9.7. 
...
Thus, the first thing to do is to reproject all datasets we will work with in the same coordinate system - EPSG 28355.
...
If you selected the output to be **[Save to temporary file]** then there will be a raster layer named `Reprojected` in the Layers Panel. This is a memory layer and you can rename this layer to Reprojected_HRSL_AUS_Tasmania_Population and save it to make it persistent.
...
Now, we can proceed to reprojecting the mosaic - one file, instead of 6 files. Go to **Raster ‣ projection ‣ Wrap (reproject)** and set the known parameters: 
   ...
   * Target CRS: EPSG:28355 
   * Resampling method: Nearest neighbour
   * Output file resolution - 30 m. 

At his point, we should have all layers in the same CRS - EPSG 28355. 
...
Files: /Users/codrinamariailie/Desktop/australia/module9/data/tasmania_srtm.tif
       /Users/codrinamariailie/Desktop/australia/module9/data/tasmania_srtm.tif.aux.xml
Size is 19547, 21175
...
PROJCRS["GDA94 / MGA zone 55",
    ...
    USAGE[
        SCOPE["unknown"],
        AREA["Australia - 144°E to 150°E"],
        BBOX[-50.89,144,-9.23,150.01]],
    ID["EPSG",28355]]
Data axis to CRS axis mapping: 1,2
Origin = (153561.247985058755148,5683238.638432537205517)
Pixel Size = (26.583842147207722,-26.583842147207722)
Metadata:
  AREA_OR_POINT=Area
Image Structure Metadata:
  INTERLEAVE=BAND
Corner Coordinates:
Upper Left  (  153561.248, 5683238.638) (143d 0'13.38"E, 38d55'52.99"S)
Lower Left  (  153561.248, 5120325.781) (142d40'47.65"E, 43d59'18.57"S)
Upper Right (  673195.610, 5683238.638) (148d59'58.45"E, 38d58'57.82"S)
Lower Right (  673195.610, 5120325.781) (149d 9'43.32"E, 44d 2'59.20"S)
Center      (  413378.429, 5401782.210) (145d57'42.00"E, 41d31'51.33"S)
Band 1 Block=19547x1 Type=Float32, ColorInterp=Gray
  Min=-74.000 Max=1612.000 
  Minimum=-74.000, Maximum=1612.000, Mean=85.744, StdDev=212.471
  Metadata:
    STATISTICS_MAXIMUM=1612
    STATISTICS_MEAN=85.74366885037
    STATISTICS_MINIMUM=-74
    STATISTICS_STDDEV=212.47116334948
...
![Zooming in on the SRTM_28355 computed histogram](media/fig918.png "Zooming in on the srtm file computed histogram")

Figure 9.18 - Zooming in on the srtm file computed histogram
...
To work only with raster layers that are relevant to Tasmania, we will use the vector extent layer (tasmania) to cut/clip all relevant raster layers. Go to **Raster ‣ Extraction ‣ Clip Raster by Mask Layer** (see figure 9.22). Similarly, you can search for Clip in the Processing Toolbar or the Locator bar.
...
![Batch process cliping all required raster layers by Tasmania division geometry](media/fig923.png "Batch process cliping all required raster layers by Tasmania division geometry")

Figure 9.23 - Batch process cliping all required raster layers by Tasmania division geometry

The parameters setup are the following:
* mask layer: tasmania (gpkg)
* both source and target CRS is EPSG 28355
* select yes to: `match the extent of the clipped raster to the mask layer` and `keep resolution of input layer`. 
* Be aware, for the srtm mosaic we will also select yes` to create an output alpha band`. Load layers at completion. 

If everything went smoothly, your QGIS main window should look like in figure 9.24. 


![Raster layers clipped by Tasmania contour](media/fig924.png "Raster layers clipped by Tasmania contour")

Figure 9.24 - Raster layers clipped by Tasmania contour.  

Now, imagine that you have to present a report on where most people are living but with consideration to the altitude[^4]. You must know how many people live between 0 and 200 m altitude in Tasmania. There are a few elements to consider. Firstly, what is the data we will use and what are its characteristics. For population, we have the High Resolution Settlement Layer Data and for relief, we have the ALOS World 3D - 30m (AW3D30). Both raster layers have a spatial resolution of 30m, which allows us to proceed to other considerations. Relief is a continuous phenomena, the spread of the population is not, yet the report would make no sense to be done by the 30m pixel. We need to identify all the pixels with cell values from 0 to 200. Considering the histogram of the srtm for our region of interest, we’ve seen that most cell values are between 0 and 400m. We can proceed to making a basic relief map based on the following divisions: 
...
![clipped_srtm representation](media/fig925.png "clipped_srtm representation")

Figure 9.25 - clipped_srtm representation

In order to calculate the number of people based on the raster data HRSL that live up to 200 meters in Tasmania, we must see which pixels fall in each of those categories. To do that, we will use **Raster Calculator**. This is a functionality allowing the user to perform calculations on the basis of existing raster pixel values. The results are written to a new raster layer in a GDAL[^5]-supported format.
...
In this window, we can recognize the operations detailed in the Concepts section, substractions, additions, comparisons and all the others (see page 3). **srtm@1** The naming convention for the rasters can be observed: what comes before the @ is the name of the raster layer, what comes after the @ is the number of the band.


Next, we will ‘slice’ our srtm raster layer to extract only the pixels with values up to 200 meters. We know that the value cells of the srtm represent a continuous numerical data (not discrete values, such as LandCover). Therefore, the operation we need to employ in this case is a comparison one - cell values <= 200 meters. To obtain this, we will write the following the Raster Calculator: 

```
"srtm@1" <= 20
``` 

Set the **Reference layer** as **srtm@1**. Your Raster Calculator should look like in figure 9.27.
...
Going further, we can show the spatial distribution of population at the 30 m spatial resolution only in this specific geographical region, we’ve selected - Tasmania division, below 200m. To do that, we again employ Raster Calculator. 

The formula is fairly simple, given that all srtn cell values we are interested in, have value 1. 

Open the Calculator and insert the following formula: 

```
"srtm_below200@1" * "Reprojected_HRSL_AUS_Tasmania_Population@1"
```
...
As opposed to previous raster calculator use, we have used 2 different raster datasets to obtain the desired result, yet you will observe that even if there are pixels that fall outside the srtm_below200.tif in the result, their value is 0. Use Value Tool to check (see figure 9.31). 
...
You can see that even if Reprojected_HRSL_AUS_Tasmania_Population has values in this specific mouse location, the raster obtained with Raster Calculator pop_below200m has value 0. 

Next, we present the spatial distribution of the population that lives below 200m in Tasmania. To choose an appropriate classification, we calculate the histogram. We can notice that most values are between 0.1 and 0.5 people per 30m. The classification we’ve chosen is visible in figure 9.32. 


![Distribution of population that lives below 200m in Tasmania, represented at a 30m resolution](media/fig932.png "Distribution of population that lives below 200m in Tasmania, represented at a 30m resolution")

Figure 9.32 - Distribution of population that lives below 200m in Tasmania, represented at a 30m resolution. 

If we are interested in the total number of people living below 200m in Tasmania and not the geographical distribution per 30 m spatial resolution, then we need to sum up all pixel values of the raster layer output.
...
And with this final step, we answered our exercise, how many people (and where) are living below 200m in Tasmania. 
...
![alt_text](media/fig940.png "Sentinel-2 cloudless layer for 2019 by EOX - 4326 for Tasmania")

Figure 9.40  - Sentinel-2 cloudless layer for 2019 by EOX - 4326 for Tasmania
...
* input layer: LandCover 2019_28355, 
* Source CRS and Target CRS: EPSG: 28355, 
* Resampling method: Nearest Neighbour, 
* No data: 255, output file resolution: 30, 
* Output data type: use input layer data type, 
* georeference extent: select the LandCover 2019 layer. 
...
Considering the extent of your study area, this operation can be significantly lengthy in time. Figure 9.54 shows how many points we have obtained for Tasmania.
...
![alt_text](media/fig956.png "Raster tiles for the Tasmania division HRSL")

Figure 9.56 - Raster tiles for the Tasmania division HRSL 
