...
You’ll notice that the expression has a number of predicates like the comparison signs (=, !=), the logical predicate (AND) and a string which is enclosed in single quotes (‘ ‘). There are also two attribute names(amenity, phone) and their values (clinic, ''). 

![alt_text](media/select.png "image_tooltip")

Figure 6.8: Select by Expression builder dialogue

3. Two Clinics are selected. You can see the selection highlighted in yellow. The selected clinics are also highlighted in the attribute table. Now we know there are two clinics in our dataset that have phone numbers.
