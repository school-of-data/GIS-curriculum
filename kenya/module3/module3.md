...
*   kiambu_admin_boundary and kiambu_juja_subcounty vector layers (inside [module3.gpkg](https://drive.google.com/drive/u/1/folders/1Yg-Hwmsf488tvq7B6tHDtiX-k9pPYG9m)).
...
In fact, there exists OpenStreetMap Kenya which is a group of local volunteers building the local mapping community and HOT or the Humanitarian OpenStreetMap Team ([https://www.hotosm.org/](https://www.hotosm.org/)) which is an international team dedicated to humanitarian action and community development through open mapping. 
...
or this exercise, we will load road network data and locations of fast food chains from OSM that can be found in Kiambu County using the QuickOSM plugin.
...
1. Load the kiambu_admin_boundary and kiambu_juja_subcounty vector layers found inside the module3 geopackage.

![Kiambu layers loaded in QGIS](media/quickosm-1.png "Kiambu layers loaded in QGIS")

Figure 3.2. Kiambu layers loaded in QGIS
...
4. Load all highways inside the extent of the kiambu_juja_subcounty layer.

    * Key: highway
    * Value: &lt;blank> (blank means ALL)
    * Layer Extent: kiambu_juja_subcounty
    * Advanced:
        * Check Node, Way, Relation, Lines, Multilinestrings

![Load all highways in the kiambu_juja_subcounty layer extent](media/quickosm-3.png "Load all highways in the kiambu layer extent")

Figure 3.4. Load all highways in the kiambu_juja_subcounty layer extent
...
8. Next, let’s load all fast food restaurant locations in the kiambu_admin_bounadry layer extent. Open the QuickOSM plugin and put the following parameters in the Quick query tab:

    * Key: amenity
    * Value: fast_food
    * Layer Extent: kiambu_admin_boundary
    * Advanced:
        * Check Node, Way, Relation, Points

![Load amenities (points) tagged as fast_food in the kiambu_admin_boundary layer extent](media/quickosm-6.png "Load amenities (points) tagged as fast_food in the kiambu_admin_boundary layer extent")

Figure 3.7: Load amenities (points) tagged as fast_food in the kiambu_admin_boundary layer extent
...
1. Load the **kenya_kiambu_population_density** raster file in QGIS.

![The HRSL for Kiambu County, Kenya](media/hrsl-1.png "The HRSL for Kiambu County, Kenya")

Figure 3.10: The HRSL for Kiambu County, Kenya
...
![Amenities tagged as fast food whose name is  loaded from OSM](media/overpass-3.png "Amenities tagged as fast food whose name is KFC loaded from OSM")
