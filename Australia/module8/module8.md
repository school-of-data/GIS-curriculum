...
* [module8.gpkg](data/module8.gpkg) which contains the following layers:
   ...
   * buildings (polygon)
   * landuse (polygon)
   * tasmania_level1 (polygon)
   * tasmania_level2 (polygon)
* The coordinate reference system used is the Geocentric Datum of Australia 1994/ Map Grid of Australia zone 55 (GDA94 / MGA zone 55)  EPSG 28355. As it is a projected coordinate system, it allows geometric calculations. 
...
The data used is spatial - it has a very well defined location on Earth, its attributes are also well identified. Thus, a point with longitude and latitude and the attribute name= Hobart Airport - represents the starting point A and a point with another pair of longitude and latitude and the attribute name= Clegg Hotel represents the end point B. Streets are represented by lines composed of segments and vertices (represented by red croses in figure 8.2), with attributes such as name, direction, speed limitation etc. 
...
Open QGIS, set up the coordinate reference system you will work in - EPSG 28355 -  and add the following data layers:
...
**Check!** All layers are in the same coordinate system (EPSG 28355) by looking in the right down side corner. If it is so, then you are looking at 8 vector data layers overlaid. 

#### **Step 2. Understand what you are looking at.** 

At this point, we have 8 vector layers loaded into our QGIS project. The next steps will help us understand our data. 
...
Click on the down right corner of the QGIS map window, on EPSG 28355 and a window as the one in figure 8.6 will appear. 
...
Minimum value: 0.02
Maximum value: 53224.87
Range: 53224.850000000006
Sum: 42470422.789999925
Mean value: 623.7211829583493
Median value: 160.55
Standard deviation: 1612.4318364803123
Coefficient of Variation: 2.585180494964826
Minority (rarest occurring value): 0.02
Majority (most frequently occurring value): 20.07
First quartile: 62.795
Third quartile: 478.54
Interquartile Range (IQR): 415.745
```


From these basic statistics, we find out that there are 68092 road segments in the loaded layer, where the longest is 53224.87m - about 53 km. We find out that the sum of roads in Tasmania is almost 42.5k km (42470422.789999925 m). Given that the mean is greater than the median, it tells us that the 2nd half of the dataset contains longer road segments and that it outweighs the road segments in the 1st half. The 'Majority (most frequently occurring value)' stats shows that most road segments have length around 20 m. 

Running the basic statistics on the layer Landuse for the fclass category, we obtain the followings: 


```
Analyzed field: fclass
Count: 11050
Unique values: 20
NULL (missing) values: 0
Minimum value: allotments
Maximum value: vineyard
Minimum length: 4
Maximum length: 17
Mean length: 7.3200904977375565
```


The results don’t look the same, we don’t have mean, nor median or standard deviation. That is because the attribute field we ran the algorithm on is different, we don’t have numbers but words - types of landuse. We find out that all of 11050 landuse parcels in Tasmania have assigned a type of the landuse. We also find out that there are 20 unique categories. 
...
We will set a number of rules for the layers we have loaded in our QGIS project, considering the real world objects they depict- roads, buildings, waterways in the state  of Tasmania. 
...
After running the algorithm, the functionality window presents the results, it has identified 33 duplicate points, just as the topology checker, and it informs the user that it has deleted it, leaving the points of interest layer with **5747 features**. Rename this layer to **pois_cleaned**. Note that the output is a layer in memory, you can make it permanent by saving it to a file.
...
Let us run another topology check, this time on our landuse layer. Configure the following rules: 

* No duplicate
* No invalid geometries

![Topology check on the buildings vector layer](media/fig817_a.png "Topology check on the landuse vector layer")

Figure 8.17a - Topology checker rules on the landuse vector layer

Run the algorithm. 

The result should look like figure 8.17b. 

![Results of topology check on the landuse vector layer](media/fig817_b.png "Results of topology check on the landuse vector layer")

Figure 8.17b - Results of topology check on the landuse vector layer

Clean the duplicate feature by using the process indicated above (figure 8.18a)


![Remove duplicate geometries on the landuse vector layer](media/fig818_a.png "Remove duplicate geometries on the landuse vector layer")

Figure 8.18a - Remove duplicate geometries on the landuse vector layer


![Results of remove duplicate geometries on the landuse vector layer](media/fig818_b.png "Results of remove duplicate geometries on the landuse vector layer")

Figure 8.18b - Results of remove duplicate geometries on the landuse vector layer

![Results of Delete duplicate geometries](media/fig818_c.png "Results of Delete duplicate geometries")

Figure 8.18c - Results of removing duplicate geometries on the landuse vector layer
...
Let's run one more algorithm to get a sense of what the attributes for our Tasmania layers are. After we’ve identified how many features each layer has, let’s see how many and which are the unique attributes in the following cases: 

* layer landuse_cleaned - attribute fclass;
...
* layer buildings - attribute type;
...
<table>
  ...
  <tr>
   ...
   <td>;block;NULL;warehouse;storage;private;Antenna;detached;coms;shed;sports_centre;hospital;house;barn;y\;university;semidetached_house;hut;commercial;yes;office;stadium;civic;cabin;residential;train_station;Café;collapsed;social_club;greenhouse;hall;block_25;kindergarten;ruins;manufacture;church;brewery;roof;service;storage_tank;public;underground;no;commercial;terrace;stable;industrial;supermarket;construction;historic;military;school;dormitory;toilets;chapel;retail;hotel;sports_hall;silo;hangar;water;fire_station;garages;tourism;government;carport;farm_auxiliary;farm;garage;pavilion;historical;apartments;parking;grandstand
   </td>
  </tr>
  <tr>
   <td>landuse_cleaned
   ...
   <td>'grass;meadow;orchard;allotments;residential;farmyard;vineyard;park;scrub;quarry;industrial;commercial;heath;military;farmland;retail;recreation_ground;nature_reserve;cemetery;forest
   </td>
  </tr>
  <tr>
   <td>pofw
   ...
   <td>	christian_lutheran;christian;christian_methodist;christian_anglican;christian_catholic
   </td>
  </tr>
  <tr>
   <td>pois
   ...
   <td>cafe;comms_tower;pitch;nightclub;vending_parking;sports_centre;caravan_site;bicycle_rental;college;archaeological;pub;wayside_cross;police;track;post_office;fast_food;alpine_hut;viewpoint;chalet;bank;fire_station;beauty_shop;sports_shop;telephone;tower;veterinary;shoe_shop;furniture_shop;camp_site;jeweller;bar;lighthouse;attraction;computer_shop;dentist;hospital;town_hall;hairdresser;travel_agent;dog_park;fountain;park;kindergarten;waste_basket;arts_centre;bench;windmill;hotel;department_store;optician;toy_shop;vending_any;wastewater_plant;museum;courthouse;beverages;newsagent;memorial;bicycle_shop;community_centre;recycling;car_dealership;university;water_tower;bakery;atm;clothes;guesthouse;recycling_glass;doityourself;nursing_home;bookshop;cinema;restaurant;pharmacy;kiosk;drinking_water;laundry;supermarket;school;library;picnic_site;mobile_phone_shop;florist;car_rental;chemist;zoo;post_box;video_shop;shelter;artwork;theatre;doctors;monument;stationery;greengrocer;tourist_info;recycling_clothes;water_well;swimming_pool;outdoor_shop;ruins;butcher;convenience;motel;toilet;playground;observation_tower;hostel;car_wash;garden_centre;camera_surveillance;gift_shop
   </td>
  </tr>
  <tr>
   <td> roads 
   ...
   <td>track_grade2;steps;living_street;primary_link;motorway_link;tertiary;path;residential;trunk_link;pedestrian;track;unclassified;tertiary_link;track_grade5;motorway;service;trunk;bridleway;secondary_link;cycleway;track_grade4;track_grade1;track_grade3;primary;footway;secondary
   </td>
  </tr>
  <tr>
   <td>waterways
...
Looking at the result, we can extract important insights regarding our data. For example, recorded as residential in Tasmania, we have 5174 buildings with a total surface area built of 879531 square meters, approx. 88 hectares. We also find out that the largest has 4402 square meters - almost 0.5 hectare and the smallest has 11 square meters. And one can continue the analysis for further valuable information. 
...
**Buffer.** Imagine that you need to analyse a new piece of legislation that asks that on an area of 500 meters around places of worship there can be no other construction built. You would want to see where exactly those delinations are and maybe even how many square meters that is for your state . First step is to define a buffer around the places of worship: **Vector ‣ geoprocessing tools ‣ Buffer**. When the buffer window opens, set the parameters like in figure 8.23: 
...
**Clip.** Imagine you want to know where all the industrial delineated areas are in your state  and also how many buildings are within that perimeter. Visual inspecting your vector data, you notice that you have a number of industrial areas that contain several buildings. You want to separate those buildings and to use them further. First step is to select all features in the landuse_cleaned layer that have as attribute industrial (see module 6 for how to do that). Afterwards, you go to **Vector ‣ Geoprocessing tools ‣ Clip** and choose as the layer to be clipped buildings. Your results should look like in figure 8.27b. 
...
Run the Clip algorithm. Make sure to check **Selected features only** box for the Overlay layer (landuse_cleaned). This will ensure that only the currently selected features will be used for clipping and speed up the computations.
...
**Thiessen (Voronoi) polygons.** Imagine you have to make a series of administrative decisions in your state  based on how many schools there are and what specific areas they serve. Geospatial analysis can be of assistance. You can start by calculating the Thiessen polygons. Based on an area containing at least two points, a Thiessen Polygon is a 2-dimensional shape which boundaries contain all space which is closer to a point within the area than any other point without the area. A good use example is in meteorology, where weather stations are discrete points, yet the information collected is considered to be measured out on the surface based on the thiessen polygons. 

To respond to the above question, we will run the algorithm only for points that have the attribute school at type. Thus, make the selection as instructed in module 6. You should have 9 features selected on layer pois_cleaned. Go to **Vector ‣ Geometry Tools ‣ Voronoi Polygons..** After setting the parameters - select the point layer for which we want the Voronoi polygons calculated and a 30% extension so that the entire Tasmania state is contained, you should see a result like in figure 8.28d. 
...
![All schools in the pois_cleaned layer](media/fig828_b.png "All schools in the pois_cleaned layer")
...
Let us assume that you have to present a report that will allow comparisons done for units of 25X25 km over the administrative unit, including: 
...
We’ve seen that there are tools that can assist us in calculating the total surface occupied by a certain type of feature, however the first step is to create our 25X25 units - cell grids. To do that, go to: **Vector ‣ Research Tools ‣ Create grid..** Set the parameters to: 
- Grid type - Rectangle (polygon)
- Grid extent - Tasmania_level1 layer
- Horizontal spacing - 25 km
- Vertical spacing - 25 km 


![Creating a 25X25km vector grid for the Tasmania state ](media/fig829_a.png "Creating a 25X25km vector grid for the Tasmania state ")

Figure 8.29a - Create 25X25km vector grid for Tasmania state 

You should get a result like in figure 8.29b. 


![25X25km vector grid for the Tasmania state ](media/fig829_b.png "25X25km vector grid for the Tasmania state ")

Figure 8.29b - 25X25km vector grid for Tasmania  

Going further in answering the questions, we need to do the following:

1. green spaces (grass, forests) built-up space per unit ratio:

Green spaces and built-up spaces is data contained by the landuse vector layer, polygon type. To know exactly what are the ‘green spaces’ we need to see what are the categories enclosed in the dataset. For that, we run **List unique values** algorithm on the `fclass` attribute and find out that we have the following ‘green’ classes: grass; meadow; orchard; farmyard; vineyard; park; farmland; nature_reserve; forest and the following ‘built-up space’ classes: residential; industrial; commercial; military; retail. Figure 8.30b presents a visualisation of our selections: 

![Filtering green areas and built-up space in Tasmania](media/fig830_a.png "Filtering green areas and built-up space in Tasmania ")

Figure 8.30a - Filtering green areas and built-up space in Tasmania 


![Spatial distribution of the green areas and built-up space in Tasmania](media/fig830_b.png "Spatial distribution of the green areas and built-up space in Tasmania")

Figure 8.30b - Spatial distribution of the green areas and built-up space in Tasmania 

The second step to answer the requirement, is to identify how much green space and how much built-up space there is in each 25X25 km. To obtain that we will **intersect** the 2 overlaid polygon vector layers. The algorithm extracts the overlapping portions of features in the Input - the landuse_cleaned layer and overlay layer - the grid layer. Go to **Vector - Geoprocessing Tools - Intersection** or look for **Intersection in the Processing Toolbox or Locator Bar** Set the algorithm parameters as in figure 8.31.
...
Now for each 25X25 km unit, we have the landuse features that we can work with. The attribute table also stores this information, as each grid cell - unit - has a unique id, see figure 8.33. 
...
Now, that we have all landuse features per 25X25 km unit, we will continue with separating the geometries of the ones that make up the green space and built-up space as defined earlier - for each grid cell.  Thus, for green space, we will select all features that have the attribute value for `fclass:`grass, forest. In attribute table, in the expression filed write down: ` "fclass" =  'meadow' or  "fclass" =  'grass' or  "fclass" =  'nature_reserve' or  "fclass" =  'park' or  "fclass"  =  'forest'` etc. Similarly, you can just type: `"fclass" in ('grass','meadow','orchard','farmyard','vineyard','park', 'farmland','nature_reserve','forest')`. Export the selected features as green_spaces_gridded  (see module 6 for more details). Don't forget to check **Save only selected features**. The new output should have 7958 features. Do the same for the built-up space. Select the features in landuse that have the attribute value for `fclass` the following: residential, industrial, commercial, military, retail, by writing the following expression in the Expression based filter window: `"fclass" =  'retail' or  "fclass" =  'commercial' or  "fclass" =  'industrial' or  "fclass" =  'residential'.` etc. Select the filtered geometries and export as builtup_spaces_gridded. Your new output should have 2059 features. 
...
Next, calculate the area occupied by each feature of the 2 layers. Go to the attribute table of each layer and then add the geometric column area by inserting the expression `round($area,2) `in the field calculator.  (see module 6 for details, if needed). However, the 25X25km grid of the Tasmania state  has a known number of grid cells, and that is 336. Therefore, we need to summarise the areas for all types of green spaces (forests, parks etc.) an built-up spaces (commercial, residential etc.) and join it accordingly to all of the 336 grid cells. To do this, we will use the **GroupStats** plugin to sum up for each grid_id all the green categories, respectively all the built-up categories. For the green_spaced_gridded vector layer, set the parameters as in figure 8.34e. 
...
![GroupStat parameters setup to sum up the green areas per each 25X25km grid cell](media/fig834_e.png "GroupStat parameters setup to sum up the green areas per each 25X25km grid cell")

Figure 8.34e - GroupStat parameters setup to sum up the green areas per each 25X25km grid cell.
...
Moving forward, we need to join the calculated spaces - green and built-up - to each 25X25 km cell grid. For that, select the grid25km vector layer in the TOC, open the properties window and go to **Joins**. This functionality allows you to join by a common attribute field, others. In our case, using the common grid_id value we will join, the sum of the built-up areas and green spaces fro the 2 csv files obtained in the previous stage.

In the **Join** window, push on the green plus button below ![Add join layer button](media/add_join_btn.png "Add join layer button") and set the parameters like in figure 8.35c, for green spaces.


![Setting the parameters to join by common field grid_id/id the sums of green and built-up spaces for each grid cell - 25X25km unit](media/fig835_c.png "Setting the parameters to join by common field grid_id/id the sums of green and built-up spaces for each grid cell - 25X25km unit")

Figure 8.35c - Setting the parameters to join by common field grid_id/id the sums of green and built-up spaces for each grid cell - 25X25km unit.
...
![Attribute table of the grid25km vector layer containing the total areas for green and built-up spaces](media/fig836_b.png "Attribute table of the grid25km vector layer containing the total areas for green and built-up spaces")

Figure 8.36b - Attribute table of the grid25km vector layer containing the total areas for green and built-up spaces. 


As we have gathered all the needed information for green and built-up spaces in the attribute table of the grid layer, all we need to do is calculate the percentage of these spaces within the 25X25 km grid cell. We will calculate it using the field calculator, using the following expression: `round(100*green_None/625000000, 5)` and `round(100*builtup_None/625000000, 5)`. Next, we add a new field in which we calculate the report of the GreenPre/BuilupPer, and thus reaching the answer to our request: green spaces (parks, forests) built-up space per unit ratio: `round( "greenPer" / "builtupPer" , 5).` Please note that given the accuracy of the OpenStreetMap landuse dataset that we use for this exercise, having overlapping polygons (for example forest and park) is possible. This would lead to the situation in which the landuse area of a 25X25km grid cell would exceed the expected area (625000000sq meters). To correct it, change the values over 100 for the greenPer, respectively builtupPer to exactly 100. 
To have a clear overview of our dataset, in cases where there is no built-up space in the grid cell - we insert the value 1000 in the attribute table, in cases where there is no green space, we will insert value 999, while in case both values are NULL then we insert 1001.  For this we can use the expression:
...
![Percentage of green area in the 25X25km grid](media/fig837_a.png "Percentage of green area in the 25X25km grid")

Figure 8.37a - Percentage of green area in the 25X25km grid
...
![Ratio of green and built-up areas in the 25X25km grid](media/fig837_e.png "Ratio of green and built-up areas in the 25X25km grid")

Figure 8.37e - Ratio of green and built-up areas in the 25X25km grid
...
![Spatial distribution of 25X25km units with most roads](media/fig838_c.png "Spatial distribution of 25X25km units with most roads")

Figure 8.38c - Spatial distribution of 25X25km units with most roads
...
![Spatial distribution of 25X25km units with most waterways](media/fig839.png "Spatial distribution of 25X25km units with most waterway")

Figure 8.39 -  Spatial distribution of 25X25km units with most waterways 

3. total number of public buildings (schools, kindergartens, hospitals , town halls etc.) for each grid cell

To count the total number of public buildings in the 25X25 unit, we will use the pois_cleaned. First, we run **Vector ‣ Analysis Tools ‣ List unique values..** and decide which building we consider public. We will select from our vector point data layer (pois) the following features: `"fclass" = 'town_hall' or "fclass" = 'hospital' or "fclass" = 'doctors' or "fclass" = 'fire_station' or "fclass" = 'community_centre' or "fclass" = 'museum' or "fclass" = 'school' or "fclass" = 'theatre'.` etc. Similarly, you can just type ` "fclass" in ( 'college',  'community_centre',  'courthouse',  'doctors' ,  'fire_station' ,  'hospital' ,  'kindergarten' ,  'library' ,  'museum' ,  'police' ,  'school', 'swimming_pool' ,  'theatre',  'university' )`. Your selection should have 201 features in total. 
...
![Count public POIs in each 25X25km grid](media/fig840_c.png "Count public POIs in each 25X25km grid")

Figure 8.40c - Count public POIs in each 25X25km grid


![Spatial distribution of public POIs density per unit 25X25km](media/fig840_d.png "Spatial distribution of public POIs density per unit 25X25km")


Figure 8.40d - Spatial distribution of public POIs density per unit 25X25km
...
Diving into the math behind each interpolation algorithm is beyond the scope of this module. However, for demonstration purposes, we will simulate precipitation data interpolation to obtain a seamless dataset on the quantity of precipitations fallen in our area of interest,Tasmania state. 
...
Now that we have our imaginary meteo stations that measure precipitations in Tasmania, we will continue by adding fictitious measurements on the course of 7 days. 
...
![Fictitious precipitation data for the 93 fictitious meteo stations in Tasmania](media/fig845.png "Fictitious precipitation data for the 93 fictitious meteo stations in Tasmania")

Figure 8.45 - Fictitious precipitation data for the 93 fictitious meteo stations in Tasmania.

Next, we will interpolate these values for each of the 7 days to obtain a seamless layer that covers the entire territory of the state . Given that the operation is repetitive, we will use batch processing. The selected interpolation method selected - strictly for demonstration purposes! - is IDW - inverse distance weighted. 

Set the following parameters: 
* distance coefficient: 2
* extent: Tasmania_level1
...
The meteo stations are visible on the map canvas and in the TOC you can see all the 7 newly created raster datasets that represent precipitation values for each day in Tasmania. 
...
However, as we have interpolated values for 7 days, let us prepare a short animation on how precipitation values have evolved for in Tasmania. 
